...
//            touchAuthenticationButton.isUserInteractionEnabled = enable
    ...
    open override func awakeFromNib() {
        ...
        /* User use passcode screen or not
         * if user use it just disable close button
         * user not use it just enable close button
         */
        if(UserDefaults.standard.bool(forKey: "getandsetonoffpasscodelock")){
            touchAuthenticationButton.isUserInteractionEnabled = false
            touchAuthenticationButton.setTitle("", for: UIControlState.normal)
        }else{
            touchAuthenticationButton.isUserInteractionEnabled = true
            touchAuthenticationButton.setTitle("Close", for: UIControlState.normal)
        }
    }
    ...
    @IBAction func touchAuthenticationAction(_ sender: UIButton) {
        self.delegate?.touchAuthenticationComplete(self, success: true, error: nil)
    }
}
